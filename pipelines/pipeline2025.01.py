...
df_estados['Regiao_sigla'] = df_estados['Regiao_sigla'].fillna('Região não informada')
df_estados['Estado_sigla'] = df_estados['Estado_sigla'].fillna('Estado não informado')
...
df_precos['Valor_compra'] = df_precos['Valor_compra'].astype(str).str.replace(',', '.', regex=False)
df_precos['Valor_compra'] = pd.to_numeric(df_precos['Valor_compra'], errors='coerce')
df_precos['Valor_venda'] = df_precos['Valor_venda'].astype(str).str.replace(',', '.', regex=False)
...
df_estados.to_sql(name= tabela_estados, con=engine, index=False, if_exists= 'replace', dtype={
    "Estado_id" : Integer,
    "Regiao_sigla" : VARCHAR(50),
    "Estado_sigla" : VARCHAR(50)
})
